...
def run_multimodal_extract(transcript_text, base64_frames, api_key):
    ...
    system_prompt = """
    你是一个资深的跨境电商选品专家。请结合我提供的【视频背景语音字幕】以及【视频关键帧画面】，提取出画面和语音中真实存在的、具有商业潜力的实体物品。
    
    ⚠️ 铁律：
    1. 必须完全基于画面内容或字幕内容提取，绝对不能凭空捏造！
    2. 如果画面和字幕中完全没有明显商品，请直接输出空列表：[]
...
try:
    ...
    user_api_key = "" # 如果本地运行没配 secrets，给个空值防崩溃

# ----------------- 侧边栏控制台 -----------------
with st.sidebar:
    st.header("⚙️ 引擎控制台")
    st.markdown("API 密钥已由系统后台安全托管。")
    st.divider()
    st.markdown("🛠️ **调试开关**")
...
st.markdown("<span style='color:#666; font-size: 1.1em;'>搭载多模态视觉引擎与全球语种自适应，真正实现“所见即所得”的商业级提取。</span>", unsafe_allow_html=True)
...
if start_btn:
    if not source_type:
        ...
        st.error("⚠️ 系统未检测到后台部署的 API Key，请检查 Secrets 配置。")
    else:
